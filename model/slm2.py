...
offspring = pd.DataFrame(columns=lice.columns)
...
#    offspring.to_csv(file_path + 'offspring.txt', sep='\t', mode='a')
    ...
    if sum(offspring.stage==1)>0:
        ...
        L1toL2_ents = min(L1toL2_ents, sum(offspring.stage==1)) 
    ...
    nu_cppds = offspring[(offspring.stage_age>=offspring.arrival) & (offspring.stage==2)].copy()
