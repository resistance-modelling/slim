...
lice['MF'] = np.random.choice(['F','M'],12,p=[0.45,0.55])
#Based on median development time at 10 degrees from Aldrin et al 2017
lice['stage'] = np.random.choice(['L1','L2'],12,p=[0.55,0.45]) 


p=stats.poisson.pmf(range(8),3)
p[2] = p[2] + 1-sum(p) #probs need to add up to one 
lice['stage_age'] = np.random.choice(range(8),12,p=p)

rands = ranTruncNorm(mean=-6, sd=2, low=-100000000, upp=0, length=12)
...
#Development rates------------------------------------------------------------------------------------------------------------------------
temp_now = oban_avetemp[cur_date.month-1]
L1toL2 = sum([devTimeAldrin(0.401,8.814,18.869,temp_now,i+tau) 
             for i in lice.loc[:].stage_age]) 
#replace ':' with lice['Farm']==farm & lice['Cage']==cage when they have more than 1 value
    
L3toL4 = sum([devTimeAldrin(1.305,18.934,7.945,temp_now,i+tau) 
             for i in lice.loc[:].stage_age])
L4toL5 = sum([devTimeAldrin(0.866,10.742,1.643,temp_now,i+tau) 
             for i in lice.loc[:].stage_age])
